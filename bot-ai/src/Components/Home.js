...
const Home = ({ data, handleOnClickSaveBtn, setCardData, saveBtn, cardDataDetails, showPastConversations, handlePastConversationBtn }) => {
  ...
  return (
    <div className="relative h-screen flex bg-gradient-to-r from-[#D7C7F433] to-[#9785BA33] overflow-hidden">
      <div className={`sm:w-2/12 h-1/5 bg-white z-10 ${showMenu ? "block" : "hidden"}`} id="sidebar-container">
        <SideBar handlePastConversationBtn={handlePastConversationBtn} />
      </div>
      <div className="w-full absolute right-0 h-full flex justify-center bg-pink sm:w-10/12" id="header-inputField">
        <div className="flex w-full h-full flex-col justify-between gap-y-8">
          <div className="flex justify-start items-center gap-x-3" id='Toggle-icons'>
            {window.innerWidth < 640 ? (
              <button onClick={handleHamBurgerBtn} className="w-6 h-6 mt-4 block sm:hidden z-30">
                {showMenu ? (
                  <ClearIcon sx={{ color: "#9785BA", height: "24px", width: "24px" }} />
                ) : (
                  <MenuIcon sx={{ color: "#9785BA", height: "24px", width: "24px" }} />
                )}
              </button>
            ) : null}
            <div className="font-ubuntu text-3xl font-bold text-[#9785BA] mt-3 mx-2 z-0" id="botAI-heading">
              Bot AI
            </div>
          </div>
